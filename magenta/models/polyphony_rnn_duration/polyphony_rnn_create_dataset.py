...
class PolyphonicSequenceExtractor(pipeline.Pipeline):
  """Extracts polyphonic tracks from a NoteSequence."""

  def __init__(self, min_secs, max_secs, name=None):
...
def get_pipeline(config, min_secs, max_secs, eval_ratio):
  """Returns the Pipeline instance which creates the RNN dataset.

  Args:
    config: An EventSequenceRnnConfig.
    min_secs: Minimum number of secs for an extracted sequence.
    max_secs: Maximum number of secs for an extracted sequence.
  ...
  for mode in ['eval', 'training']:
    ...
    poly_extractor = PolyphonicSequenceExtractor(
        min_secs=min_secs, max_secs=max_secs, name='PolyExtractor_' + mode)
